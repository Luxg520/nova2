...
namespace Nova
{
    ...
    public abstract class SerializableData : ISerializable
    {
        ...
        protected virtual int StrISerDicCount { get { return 0; } }
        ...
        protected Dictionary<string, ISerializable>[] sisDictData
        {
            get
            {
                if (_sisDictData == null)
                {
                    _sisDictData = new Dictionary<string, ISerializable>[StrISerDicCount];
                    for (int i = 0; i < StrISerDicCount; i++)
                        _sisDictData[i] = new Dictionary<string, ISerializable>();
                }
                return _sisDictData;
            }
        }Dictionary<string, ISerializable>[] _sisDictData = null;
        ...
        // 反序列化时，需要额外的函数帮助确定到底要创建什么类型的对象
        protected virtual Func<ISerializable>[] SISDictDataCreators
        {
            get
            {
                return null;
        ...
        public virtual void Serialize(IWriteableBuffer buff)
        {
            ...
            //buff.Write(sisDictData.Length);
            //for (int i = 0; i < sisDictData.Length; i++)
            //{
            //    Dictionary<string, ISerializable> dict = sisDictData[i];
            //    string[] ks = new string[dict.Count];
            //    ISerializable[] vs = new ISerializable[dict.Count];
            //    dict.Keys.CopyTo(ks, 0);
            //    dict.Values.CopyTo(vs, 0);
        ...
        public int TryReadInt(IReadableBuffer data)
        {
        ...
        public virtual void Deserialize(IReadableBuffer data)
        {
            ...
            // string-ISerializable dict
            //cnt = TryReadInt(data);
            //for (int i = 0; i < cnt; i++)
            //{
            //    string[] ks = data.ReadStringArr();
            //    ISerializable[] vs = data.ReadArr(SISDictDataCreators[i]);
            //    Dictionary<string, ISerializable> dict = new Dictionary<string, ISerializable>();
            //    for (int j = 0; j < ks.Length; j++)
            //        dict[ks[j]] = vs[j];
